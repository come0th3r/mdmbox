...
#include "preview/mdmbox_shell_host.h"
#include "preview/mdmbox_shell_window.h"
...
MainWindow::MainWindow(QWidget *parent) : QMainWindow(parent), ui(new Ui::MainWindow) {
    ...
    // Hide ads and update buttons — not needed in MDMBOX
    ui->toolButton_ads->setVisible(false);
    ui->toolButton_update->setVisible(false);
    
    // Add "Return to MDMBOX" button
    {
        auto btnReturnMdm = new QToolButton(this);
        btnReturnMdm->setText(tr("← MDMBOX"));
        btnReturnMdm->setToolButtonStyle(Qt::ToolButtonTextOnly);
        btnReturnMdm->setStyleSheet(
            "QToolButton { background-color: #0078d4; color: white; border-radius: 6px;"
            " font-weight: bold; font-size: 11px; padding: 4px 12px; border: none; }"
            "QToolButton:hover { background-color: #005faa; }"
        );
        connect(btnReturnMdm, &QToolButton::clicked, this, [=] {
            this->hide();
            UpdateMdmBoxShell();
            ShowMdmBoxShell();
        });
        // Insert into the toolbar layout before the horizontal spacer
        auto topLayout = qobject_cast<QHBoxLayout*>(ui->centralwidget->layout()->itemAt(0)->layout());
        if (topLayout) topLayout->insertWidget(0, btnReturnMdm);
    }
    ...
    connect(tray, &QSystemTrayIcon::activated, this, [=](QSystemTrayIcon::ActivationReason reason) {
        if (reason == QSystemTrayIcon::Trigger) {
            auto *shell = GetMdmBoxShellWindow();
    ...
    if (NekoGui::dataStore->vpn_implementation != 1) {
    ...
    if (!NekoGui::dataStore->remember_spmode.isEmpty() || NekoGui::dataStore->flag_restart_tun_on || NekoGui::dataStore->remember_enable) {
...
QTableWidget *MainWindow::getProxyTable() {
    return ui->proxyListTable;
}

QString MainWindow::shellTrafficText() const {
    return traffic_update_cache.trimmed();
}

int MainWindow::shellLatencyMs() const {
    if (running != nullptr) return running->latency;
    if (NekoGui::dataStore && NekoGui::dataStore->selected_id >= 0 && NekoGui::profileManager) {
        if (auto selected = NekoGui::profileManager->GetProfile(NekoGui::dataStore->selected_id)) {
...
QJsonObject MainWindow::shellSignalStats() const {
    const int activeProfileId = NekoGui::dataStore
        ? (NekoGui::dataStore->started_id >= 0 ? NekoGui::dataStore->started_id : NekoGui::dataStore->selected_id)
        : -1;

    QVector<int> samples;
    if (activeProfileId >= 0 && activeProfileId == shell_signal_profile_id) {
        samples = shell_signal_samples;
    }

    const int fallbackLatency = shellLatencyMs();
    if (samples.isEmpty() && fallbackLatency > 0) samples.append(fallbackLatency);

    int successCount = 0;
    int failureCount = 0;
    int sumLatency = 0;
    int validCount = 0;
    QVector<int> validSamples;
    ...
    const int totalCount = successCount + failureCount;
    const int avgPing = validCount > 0 ? qRound(double(sumLatency) / double(validCount)) : 0;

    int jitter = 0;
    if (validSamples.size() > 1) {
        int diffSum = 0;
        for (int i = 1; i < validSamples.size(); ++i) {
            diffSum += qAbs(validSamples[i] - validSamples[i - 1]);
        }
        jitter = qRound(double(diffSum) / double(validSamples.size() - 1));
    }

    const double successRate = totalCount > 0 ? (double(successCount) * 100.0 / double(totalCount)) : 0.0;

    int score = 100;
    if (avgPing > 0) score -= qMin(45, qMax(0, (avgPing - 20) / 4));
    score -= qMin(25, jitter);
    if (totalCount > 0) score -= qRound((100.0 - successRate) * 0.9);
    if (validCount == 0) score = 0;
    score = qBound(0, score, 100);

    QString grade = tr("Нет данных");
    QString subtitle = tr("Запустите проверку узла");
    QString accent = QStringLiteral("#94a3b8");
    if (score >= 90) {
        grade = tr("Отлично");
        accent = QStringLiteral("#16a34a");
    } else if (score >= 75) {
        grade = tr("Хорошо");
        accent = QStringLiteral("#22c55e");
    } else if (score >= 55) {
        grade = tr("Средне");
        accent = QStringLiteral("#d97706");
    } else if (score >= 30) {
        grade = tr("Плохо");
        accent = QStringLiteral("#f97316");
    } else if (totalCount > 0) {
        grade = tr("Нестабильно");
        accent = QStringLiteral("#dc2626");
    }

    if (validCount > 0 && totalCount > 0) {
        subtitle = tr("Пинг %1 мс • jitter %2 мс • успех %3%")
                       .arg(avgPing)
                       .arg(jitter)
                       .arg(QString::number(successRate, 'f', totalCount >= 10 ? 1 : 0));
    } else if (validCount > 0) {
        subtitle = tr("По последней проверке: %1 мс").arg(avgPing);
    }
    ...
    return QJsonObject{
        ...
        {QStringLiteral("successRate"), successRate},
        {QStringLiteral("avgPing"), avgPing},
        {QStringLiteral("jitter"), jitter},
        {QStringLiteral("samples"), totalCount},
...
void MainWindow::shellMaybeScheduleSignalProbe() {
    ...
    if (shell_signal_last_probe_time.isValid() && shell_signal_last_probe_time.secsTo(QTime::currentTime()) < 45) return;

    shell_signal_probe_in_flight = true;
    shell_signal_last_probe_time = QTime::currentTime();
    speedtest_current();
}

void MainWindow::shellRecordSignalSample(int profileId, int latencyMs, bool success) {
    if (profileId < 0) {
        shell_signal_probe_in_flight = false;
        return;
    }

    if (shell_signal_profile_id != profileId) {
        shell_signal_profile_id = profileId;
        shell_signal_samples.clear();
    }

    shell_signal_samples.append(success && latencyMs > 0 ? latencyMs : -1);
    while (shell_signal_samples.size() > 20) shell_signal_samples.removeFirst();
...
void MainWindow::neko_set_spmode_vpn(bool enable, bool save) {
    if (enable && NekoGui::dataStore->vpn_implementation != 1) {
        NekoGui::dataStore->vpn_implementation = 1;
        NekoGui::dataStore->Save();
    }
