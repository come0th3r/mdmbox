...
void MainWindow::speedtest_current() {
#ifndef NKR_NO_GRPC
    const int currentProfileId = NekoGui::dataStore
    ...
    runOnNewThread([=] {
        ...
        if (!rpcOK) {
            runOnUiThread([=] {
                shellRecordSignalSample(currentProfileId, 0, false);
        ...
        runOnUiThread([=] {
            shellRecordSignalSample(currentProfileId, latency, result.error().empty() && latency > 0);
            if (!result.error().empty()) {
