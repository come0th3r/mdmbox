...
namespace NekoGui {
    namespace {
        struct ProcessMatchRule {
            QStringList processName;
            QStringList processPath;
        };
        ...
        void appendUniqueCaseInsensitive(QStringList &list, const QString &value) {
            ...
            for (const auto &item: list) {
                if (item.compare(trimmed, Qt::CaseInsensitive) == 0) return;
            }
            list << trimmed;
        }

        bool looksLikeProcessPath(const QString &value) {
            auto normalized = value.trimmed();
            return normalized.contains('/') ||
                   normalized.contains('\\') ||
                   QRegularExpression("^[A-Za-z]:").match(normalized).hasMatch() ||
                   normalized.startsWith("./") ||
                   normalized.startsWith("../");
        }

        ProcessMatchRule makeProcessMatchRule(const QStringList &rules) {
            ProcessMatchRule result;
            for (const auto &rule: rules) {
                auto trimmed = rule.trimmed();
                if (trimmed.isEmpty()) continue;
                if (looksLikeProcessPath(trimmed)) {
                    appendUniqueCaseInsensitive(result.processPath, QFileInfo(trimmed).absoluteFilePath());
                } else {
                    appendUniqueCaseInsensitive(result.processName, trimmed);
                }
            }
            return result;
        }

        QJsonObject buildProcessMatchRule(const QString &outbound, const ProcessMatchRule &rule) {
            QJsonObject jsonRule;
            if (rule.processName.isEmpty() && rule.processPath.isEmpty()) return jsonRule;
            jsonRule["outbound"] = outbound;
            if (!rule.processName.isEmpty()) jsonRule["process_name"] = QList2QJsonArray(rule.processName);
            if (!rule.processPath.isEmpty()) jsonRule["process_path"] = QList2QJsonArray(rule.processPath);
            return jsonRule;
        }
    } // namespace
...
#define IP_USER_RULE                                                             \
    ...
    void BuildConfigSingBox(const std::shared_ptr<BuildConfigStatus> &status) {
        ...
        if (dataStore->vpn_internal_tun && dataStore->spmode_vpn && !status->forTest) {
            ...
            inboundObj["stack"] = Preset::SingBox::VpnImplementationValue(dataStore->vpn_implementation);
        ...
        if (dataStore->vpn_internal_tun && dataStore->spmode_vpn && !status->forTest) {
            ...
            auto userProcessRule = buildProcessMatchRule(match_out, makeProcessMatchRule(SplitLinesSkipSharp(dataStore->vpn_rule_process)));
            if (!userProcessRule.isEmpty()) {
                status->routingRules += userProcessRule;
            }
            ...
            if (!autoBypassExternalProcessPaths.isEmpty()) {
                auto autoBypassRule = buildProcessMatchRule("bypass", makeProcessMatchRule(autoBypassExternalProcessPaths));
                if (!autoBypassRule.isEmpty()) {
                    status->routingRules += autoBypassRule;
                }
            }
        }
        ...
        auto ruleNeedsGeoAsset = [&](const auto &self, const QJsonValue &value, const QString &key) -> bool {
            if (value.isObject()) {
                const auto object = value.toObject();
                const auto directValue = object.value(key);
                if (directValue.isArray() && !directValue.toArray().isEmpty()) return true;
                if (directValue.isString() && !directValue.toString().trimmed().isEmpty()) return true;
                if (directValue.isObject() && !directValue.toObject().isEmpty()) return true;
                for (auto it = object.constBegin(); it != object.constEnd(); ++it) {
                    if (self(self, it.value(), key)) return true;
                }
                return false;
            }
            if (value.isArray()) {
                for (const auto &item : value.toArray()) {
                    if (self(self, item, key)) return true;
                }
            }
            return false;
        };
        const bool needGeoip = ruleNeedsGeoAsset(ruleNeedsGeoAsset, routingRules, QStringLiteral("geoip"));
        const bool needGeosite = ruleNeedsGeoAsset(ruleNeedsGeoAsset, routingRules, QStringLiteral("geosite"));
        const auto geoip = FindCoreAsset("geoip.db");
        const auto geosite = FindCoreAsset("geosite.db");
        if (needGeoip && geoip.isEmpty()) status->result->error = +"geoip.db not found";
        if (needGeosite && geosite.isEmpty()) status->result->error = +"geosite.db not found";
        ...
        if (!geoip.isEmpty()) {
            routeObj["geoip"] = QJsonObject{
                {"path", geoip},
            };
        }
        if (!geosite.isEmpty()) {
            routeObj["geosite"] = QJsonObject{
                {"path", geosite},
            };
        }
    ...
    QString WriteVPNSingBoxConfig() {
        ...
        auto userProcessRule = buildProcessMatchRule(match_out, makeProcessMatchRule(SplitLinesSkipSharp(dataStore->vpn_rule_process)));
        if (!userProcessRule.isEmpty()) {
            process_name_rule = "," + QJsonObject2QString(userProcessRule, false);
        }
        ...
        auto config = ReadFileText(configFn)
                          ...
                          .replace("%STACK%", Preset::SingBox::VpnImplementationValue(dataStore->vpn_implementation))
